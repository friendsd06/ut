...
from demo99 import TaskProcessor
    ...
    def get_dag_metadata(self, dag_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            with self.conn.cursor(cursor_factory=RealDictCursor) as cur:
                ...
                # Get dependencies
                self.logger.debug(f"Fetching task dependencies for DAG: {dag_id}")
                self._execute_query(cur, DAGQueries.GET_TASK_DEPENDENCY, (dag_id,))
                task_dependencies = cur.fetchall()
                self.logger.debug(
                    f"Successfully fetched {len(task_dependencies)} task dependencies for DAG: {dag_id}"
                )

                self.logger.debug(f"Fetching task metadata  for DAG: {dag_id}")
                self._execute_query(cur, DAGQueries.GET_TASK_META_DATA, (dag_id,))
                task_metadata = cur.fetchall()
                self.logger.debug(
                    f"Successfully fetched {len(task_metadata)} task meta DAG: {dag_id}"
                )

                # Initialize TaskProcessor
                processor = TaskProcessor(self.logger)

                results = processor.process_pipeline(task_dependencies, task_metadata)
                ...
                dependencies = next(iter(results.values()))
                dependencies = results[pipeline_id]
    ...
    def get_active_dags(self, environment: Optional[str] = None) -> List[str]:
        ...
        try:
            ...
            with self.conn.cursor(cursor_factory=RealDictCursor) as cur:
                ...
                result = cur.fetchall()
                dag_ids = [row['pipelineid'] for row in result]
