...
class TaskProcessor:
    ...
    def validate_input_data(self, pipeline_data: List[Dict[str, Any]], task_metadata: List[Dict[str, Any]]) -> None:
        ...
        required_metadata_fields = {'taskid', 'operator_type', 'dependencytype'}
    ...
    def load_task_info(self, task_metadata: List[Dict[str, Any]]) -> None:
        ...
        Raises:
            InvalidTaskError: If duplicate task IDs are found or operator/dependency type is not defined.
        """
        for idx, metadata in enumerate(task_metadata, start=1):
            task_id = metadata.get('taskid')
            operator = metadata.get('operator_type')
    ...
    def validate_task(self, task: Dict[str, Any]) -> Tuple[str, List[str]]:
        ...
        if parent_ids is None:
            ...
            raise InvalidTaskError(
                f"Invalid type for 'dependenttaskid' in task '{task_id}': {parent_ids}"
            )
    ...
    def format_dependencies(self, task: str, dep_groups: Dict[str, List[str]]) -> List[str]:
        ...
        if all(self.get_dependency_type(child_task) == 'parallel' for child_task in downstream_tasks):
    ...
    def process_single_pipeline(self, pipeline_id: str, tasks: List[Dict[str, Any]]) -> List[str]:
        ...
        # Identify tasks with no downstream or upstream (standalone)
        ...
        while queue:
            ...
            for dep_type, tks in downstream_groups.items():
                for child_task in tks:
    ...
    def visualize_dependencies(self, pipeline_id: str, tasks: List[Dict[str, Any]], output_format: str = 'png',
        ...
        # If no output_path is given, just use the pipeline_id as the base filename without extension.
        if not output_path:
            output_path = f"pipeline_{pipeline_id}"

        try:
            # Do not add the extension here; Graphviz will handle it.
            dot.render(filename=output_path, cleanup=True)
            self.logger.info(f"Dependency graph for pipeline '{pipeline_id}' saved to '{output_path}.{output_format}'.")
        except Exception as e:
...
def get_test_pipeline_data_and_metadata() -> Tuple[List[Dict[str, Any]], List[Dict[str, Any]]]:
    """
    Simulates fetching pipeline data and task metadata from a database or another source.

    Returns:
        Tuple[List[Dict[str, Any]], List[Dict[str, Any]]]:
            Pipeline data and task metadata lists.
    """
    pipeline_data = [
...
def main():
    ...
    try:
        ...
        for pipeline_id, deps in results.items():
            ...
            # Generate visualization if no errors
            if all(not dep.startswith("Error:") for dep in deps):
