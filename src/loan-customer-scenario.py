...
# COMMAND ----------
# Define schema for loan dataset (40 attributes)
loan_schema = StructType([
    ...
    StructField("property_zipcode", StringType(), True)
])

# Define schema for customer dataset (15 attributes)
customer_schema = StructType([
    ...
    StructField("social_security_number", StringType(), False),
...
def generate_customer_data(num_customers):
    return (spark.range(0, num_customers)
            ...
            .withColumn("social_security_number", expr("lpad(cast(rand() * 999999999 as int), 9, '0')"))
            .withColumn("annual_income", expr("rand() * 200000"))
            .withColumn("credit_score", expr("300 + cast(rand() * 550 as int)"))
            .withColumn("customer_since", expr("date_sub(current_date(), cast(rand() * 365 * 10 as int))"))
            .select("customer_id", "first_name", "last_name", "email", "phone_number", "address", "city", "state",
                    "zip_code", "country", "date_of_birth", "social_security_number", "annual_income", "credit_score", "customer_since")
            )
...
def generate_loan_data(num_loans, skewed_customer_id, other_customer_ids):
    ...
    other_df = (spark.range(0, other_loans)
                .withColumn("customer_id", expr(f"array({','.join(repr(id) for id in other_customer_ids)})[cast(rand() * {len(other_customer_ids)} as int)]")))

    return (skewed_df.union(other_df)
            .withColumn("loan_id", expr("uuid()"))
            ...
            .withColumn("application_date", expr("date_sub(origination_date, cast(rand() * 30 as int))"))
            .withColumn("approval_date", expr("date_add(application_date, cast(rand() * 14 as int))"))
            .withColumn("funding_date", expr("date_add(approval_date, cast(rand() * 7 as int))"))
            .withColumn("branch_id", expr("concat('BR_', lpad(cast(rand() * 1000 as int), 4, '0'))"))
            .withColumn("underwriter_id", expr("concat('UW_', lpad(cast(rand() * 500 as int), 3, '0'))"))
            .withColumn("insurance_type", expr("array(null, 'Life', 'Disability', 'Property')[cast(rand() * 4 as int)]"))
            .withColumn("insurance_premium", expr("case when insurance_type is not null then rand() * 1000 else null end"))
            .withColumn("co_borrower_id", expr("case when rand() < 0.3 then uuid() else null end"))
            .withColumn("guarantor_id", expr("case when rand() < 0.1 then uuid() else null end"))
            .withColumn("channel", expr("array('Branch', 'Online', 'Phone', 'Partner')[cast(rand() * 4 as int)]"))
            .withColumn("promotion_id", expr("case when rand() < 0.2 then concat('PROMO_', lpad(cast(rand() * 100 as int), 3, '0')) else null end"))
            .withColumn("property_zipcode", expr("lpad(cast(rand() * 99999 as int), 5, '0')"))
