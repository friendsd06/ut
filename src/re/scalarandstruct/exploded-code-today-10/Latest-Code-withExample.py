...
from pyspark.sql.functions import (
    explode_outer, col, when, lit, concat, array, concat_ws, collect_list
)
...
target_data = [
    {
        ...
        "orders": [
            {"order_id": "O1001", "parent_primary_key": "P1", "child_primary_key": "C1",
             "order_date": "2023-01-10", "amount": "250.0", "status": "Shipped"},  # amount as string
            {"order_id": "O1002", "parent_primary_key": "P1", "child_primary_key": "C1",
...
def compare_and_collect_differences(
    ...
    # Aggregate differences per key
    group_keys = global_primary_keys
    result_df = result_df.groupBy(group_keys).agg(
        concat_ws("; ", collect_list(result_col_name)).alias(result_col_name)
    )
...
# Now, create df6 and perform the join
# Sample data for df6
df6_data = [
    {
        "parent_primary_key": "P1",
...
# Define the schema for df6
df6_schema = StructType([
    StructField("parent_primary_key", StringType(), True),
    StructField("child_primary_key", StringType(), True),
    StructField("column_a", StringType(), True),
    StructField("column_b", StringType(), True),
    StructField("column_c", StringType(), True),
    StructField("column_d", StringType(), True),
    StructField("column_e", StringType(), True),
    StructField("column_f", StringType(), True)
])

# Create df6 DataFrame
df6 = spark.createDataFrame(df6_data, df6_schema)

# Ensure that the global primary keys are of the same data types in both DataFrames
# Check data types
print("unified_diff_df data types:")
print(unified_diff_df.dtypes)
print("\ndf6 data types:")
print(df6.dtypes)

# Perform the join
# Choose the join type based on your needs; let's assume a full outer join to include all records
final_df = unified_diff_df.join(
    df6,
    on=global_primary_keys,
    how='full_outer'  # Options: 'inner', 'left', 'right', 'full_outer'
)

# Display the final DataFrame after joining
print("=== Final DataFrame After Joining unified_diff_df with df6 ===")
