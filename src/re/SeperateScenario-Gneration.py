...
from pyspark.sql.functions import col, when, lit, expr, date_format
from pyspark.sql.types import StringType

# Initialize SparkSession
spark = SparkSession.builder \
    .appName("Reconciliation Scenarios Generation") \
...
# Load source data
source_df = spark.read.parquet("s3a://your-s3-bucket/source_loan_data/")
...
scenarios = [
    ...
    source_df.withColumn("balance", col("balance").cast(StringType())),
    ...
    source_df.withColumn("due_date", date_format(col("due_date"), "MM-dd-yyyy"))
]

# Save each scenario as a separate target file in S3
for i, scenario_df in enumerate(scenarios, start=1):
    scenario_df.write.mode("overwrite").parquet(f"s3a://your-s3-bucket/target_loan_data_scenario_{i}/")

print("All scenarios have been generated and saved to S3.")
