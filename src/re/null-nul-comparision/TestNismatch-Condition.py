...
target_data = [
    {
        ...
        "orders": [
            ...
            },
            {
                ...
                "order_date": "2020-02-01",
                "amount": 150.0,
                "status": "shipped"  # Status is different
            }
        ],
        "payments": [
            {
        ...
        "new_array": [
            {
                ...
                "parent_primary_key": "1",
                "child_primary_key": "A",
                "detail": "Detail2",  # Detail is different
                "extra_info": "Extra1"
            }
        ]
    }
...
def compare_and_add_mismatch(
    ...
    for field in compare_fields:
        ...
        # Create mismatch condition
        mismatch_cond = (~col(source_field).eqNullSafe(col(target_field))) | (col(source_field).isNull() ^ col(target_field).isNull())
        mismatch_conditions.append(mismatch_cond)

    # Combine differences into a single column
    combined_differences = concat_ws("; ", array(*difference_expressions))

    # Combine mismatch conditions into a single condition
    if mismatch_conditions:
        mismatch_col = reduce(lambda x, y: x | y, mismatch_conditions)
    else:
        mismatch_col = lit(False)

    # Select primary keys, mismatch column, and combined differences
...
for array_col, array_pks in array_columns.items():
    ...
    diff_df = compare_and_add_mismatch(
...
# Display all rows with mismatch status
def display_differences(difference_results):
    for array_col, diff_df in difference_results.items():
        print(f"=== All Rows in {array_col.capitalize()} ===")
        diff_df.show(truncate=False)
        print("\n")

display_differences(difference_results)

# Display only mismatched rows
def display_mismatches_only(difference_results):
    for array_col, diff_df in difference_results.items():
        print(f"=== Mismatches in {array_col.capitalize()} ===")
        diff_df.filter(col("mismatch") == True).show(truncate=False)
        print("\n")

display_mismatches_only(difference_results)
