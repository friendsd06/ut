...
from pyspark.sql.functions import col, when, lit, struct, collect_list, size, udf
from pyspark.sql.types import StructType, StructField, StringType, IntegerType, ArrayType
from typing import List, Optional

def compare_delta_tables(table1: DataFrame, table2: DataFrame, join_key: str, include_columns: Optional[List[str]] = None, exclude_columns: Optional[List[str]] = None, parent_prefix: str = "PARENT_"):
    """
    Compare two Delta tables and return a DataFrame with mismatches and counts.
    Identifies first-level parent value changes and adds a prefix.
    """
    ...
    # Function to get the first-level parent
    def get_first_level_parent(column_name):
    ...
    mismatch_struct = struct(*[
        when(col(f"t1.{c}") != col(f"t2.{c}"),
             struct(lit(c).alias("column"),
                    lit(get_first_level_parent(c)).alias("parent"),
    ...
    def explode_mismatches(mismatches):
        result = []
        parents_changed = set()
        for row in mismatches.asDict().values():
            if row is not None:
                if row['parent'] not in parents_changed:
                    row['column'] = f"{parent_prefix}{row['parent']}"
                    parents_changed.add(row['parent'])
                result.append(row)
        return result
